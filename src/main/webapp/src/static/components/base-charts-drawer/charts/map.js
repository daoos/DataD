...
export default{
    ...
    init(eCharts){
        let demoData = [
            ...
            {name:"深圳", value:87, to:"上海"},
            {name:"杭州", value:75, from:"北京"},
            ...
            series : [
                {
                    ...
                    data:this._linesData(demoData),
                },
                {
                    name: 'Top 5',
                    type: 'effectScatter',
                    coordinateSystem: 'geo',
                    data: this._convertData(demoData.sort(function (a, b) {
    ...
    options(eCharts, paramsDevelop){
        ...
        Common.start(eCharts, config.url||"/charts/map", params, config.interval)((data, isSeries=true) =>{
            ...
            option.series[1].data = this._convertData(result.sort(function (a, b) { return b.value - a.value;}).slice(0, 6));//top5
            option.series[2].data = this._convertData(result);//other
            eCharts.setOption(option);
            eCharts.hideLoading();
        });
    },
    _linesData(data){
        ...
        data.forEach(x=>{
            ...
            if(x["to"]){
                fromCoord = this.geoCoordMap[x["name"]];
                toCoord = this.geoCoordMap[x["to"]];
            }
            if(x["from"]){
                fromCoord = this.geoCoordMap[x["from"]];
                toCoord = this.geoCoordMap[x["name"]];
            }
